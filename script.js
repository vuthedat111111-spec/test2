...
const [viewMode, setViewMode] = useState('input'); // 'input' hoặc 'list'
    // -----------------
    
    // Reset về màn hình nhập mỗi khi mở Modal mới
    useEffect(() => {
        if (isOpen) setViewMode('input');
    }, [isOpen]);
    // Logic xử lý danh sách từ vựng
    const vocabItems = useMemo(() => {
        ...
        const items = words.map(word => {
            // Lấy dữ liệu gốc từ DB hệ thống
            const dbInfo = dbData?.TUVUNG_DB?.[word] || {};
            // Lấy dữ liệu người dùng đã sửa (nếu có) - Cần truyền customVocabData từ App xuống
            // Ở đây tạm thời lấy DB làm gốc
            
            // Tính âm hán tự mặc định
            const defaultHan = word.split('').map(c => dbData?.KANJI_DB?.[c]?.sound || '').filter(s => s).join(' ');

            return {
                word: word,
                reading: dbInfo.reading || '',
                meaning: dbInfo.meaning || '',
                hanviet: defaultHan,
                isMissing: !dbInfo.reading || !dbInfo.meaning // Đánh dấu nếu thiếu thông tin
            };
        });

        // Ưu tiên từ thiếu thông tin (isMissing) lên đầu
        return items.sort((a, b) => (b.isMissing ? 1 : 0) - (a.isMissing ? 1 : 0));
    }, [localText, dbData]);
    const [isFilterMenuOpen, setIsFilterMenuOpen] = useState(false);
    const filterRef = useRef(null);
    const isComposing = useRef(false);
            ...
            <div className="bg-white w-full max-w-lg sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] sm:max-h-[85vh] animate-in slide-in-from-bottom-8 sm:zoom-in-95 duration-300">
                ...
                <div className="flex items-center justify-between p-4 border-b border-gray-100 bg-gray-50/50">
                    <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest">
                        {viewMode === 'input' ? 'Bước 1: Nhập dữ liệu' : 'Bước 2: Kiểm tra & Chỉnh sửa'}
                    </h3>
                    <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full bg-white border border-gray-200 text-gray-500 hover:bg-red-50 hover:text-red-500 transition-all shadow-sm">✕</button>
                </div>

                <div className="p-6 flex-1 overflow-y-auto custom-scrollbar">
                    {viewMode === 'input' ? (
                        /* --- CHẾ ĐỘ 1: NHẬP LIỆU (Giữ nguyên các nút của bạn) --- */
                        <div className="space-y-5 animate-in fade-in duration-300">
                            {/* Thanh tìm kiếm */}
                            <SearchBar mode={mode} dbData={dbData} onSelectResult={(item) => { /* Code giữ nguyên như cũ */ }} />
                            
                            {/* Ô nhập liệu */}
                            <textarea value={localText} onChange={handleInputText} onBlur={handleBlurText} placeholder={getDynamicPlaceholder()} className="w-full h-[120px] p-4 bg-gray-50 border border-gray-200 rounded-2xl resize-none text-[18px] font-['Klee_One']" />
                            
                            {/* 3 Nút Tiện ích của bạn */}
                            <div className="grid grid-cols-3 gap-3">
                                <button onClick={() => setIsLibraryOpen(true)} className="py-4 rounded-2xl bg-white border border-gray-200 font-bold text-[10px] uppercase">Thư viện</button>
                                <button onClick={handleShuffle} className="py-4 rounded-2xl bg-white border border-gray-200 font-bold text-[10px] uppercase">Xáo trộn</button>
                                <button onClick={() => setIsFilterMenuOpen(!isFilterMenuOpen)} className="py-4 rounded-2xl bg-white border border-gray-200 font-bold text-[10px] uppercase">Bộ lọc</button>
                            </div>
                        </div>
                    ) : (
                        /* --- CHẾ ĐỘ 2: DANH SÁCH (Bảng từ vựng mới) --- */
                        <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
                            <div className="bg-orange-50 border border-orange-100 p-3 rounded-xl text-[11px] text-orange-700 font-bold flex gap-2">
                                <span>⚠️</span>
                                <span>Kiểm tra các mục màu đỏ và bổ sung thông tin nếu cần trước khi học.</span>
                            </div>
                            
                            <div className="space-y-2">
                                {vocabItems.map((item, idx) => (
                                    <div key={idx} className={`p-3 border rounded-2xl flex items-center justify-between transition-all ${item.isMissing ? 'border-red-200 bg-red-50/30' : 'border-gray-100 bg-white'}`}>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2">
                                                <span className="text-lg font-black text-gray-800">{item.word}</span>
                                                {item.isMissing && <span className="text-[9px] bg-red-500 text-white px-1.5 py-0.5 rounded-full font-black uppercase">Thiếu thông tin</span>}
                                            </div>
                                            <p className="text-[11px] text-gray-500 font-medium">
                                                <span className="text-indigo-600">{item.hanviet}</span> • {item.reading || '...'} • {item.meaning || '...'}
                                            </p>
                                        </div>
                                        {/* Nút sửa - Bạn cần nối logic setEditingVocab vào đây */}
                                        <button className="p-2 hover:bg-gray-100 rounded-xl text-gray-400 hover:text-indigo-600 transition-all">
                ...
                <div className="p-5 border-t border-gray-100 bg-white">
                    {viewMode === 'input' ? (
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setViewMode('input')} className="flex-1 py-4 bg-gray-100 text-gray-500 font-bold rounded-2xl active:scale-95 text-xs uppercase">Quay lại</button>
                            <button 
                                onClick={() => onStart(targetAction)}
                                className="flex-[2] py-4 bg-gray-900 text-white font-black rounded-2xl shadow-xl active:scale-95 transition-all uppercase tracking-widest"
                            >
                                BẮT ĐẦU HỌC 🚀
                            </button>
                        </div>
